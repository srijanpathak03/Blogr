...
import { SigninInput } from '@srijanpathak/blog-common';
...
export const Signin = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLoginInputs({ ...loginInputs, [id]: value });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${BACKEND_URL}/api/v1/user/signin`, loginInputs);
      const {jwt,userId} = await response.data;
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-gray-200 via-gray-300 to-gray-400 p-4 sm:p-6 lg:p-8">
      <div className="w-full max-w-md bg-white shadow-lg rounded-lg overflow-hidden">
        <form onSubmit={handleSubmit} className="p-8">
          ...
          <p className="mt-2 text-sm text-gray-600 text-center">Enter your credentials to access your account.</p>
